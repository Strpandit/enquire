module Chat
  class ConversationAccess
    ...
    def self.ensure_customer_can_start!(customer:, business_profile:)
      ...
      raise Error, "Chat pricing is not configured" unless business_profile.chat_price.to_i.positive?
      raise Error, "Insufficient wallet balance for one minute chat" if customer.wallet_balance < business_profile.chat_price
    end
